...
- Env vars (see `.env.example`, real values in untracked `.env`): `AUTH_SECRET` (Auth.js session/cookie signing) and `API_MOCKING` (`enabled` starts MSW via `instrumentation.ts`). **No Smarkets credentials belong in env** — they are form-only per `TASK.md`.
